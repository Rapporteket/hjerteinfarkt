...
output:
  ...
  html_document:
    toc: false
params:
  type: "pdf"        # Shiny vil overskrive denne
  var: "mpg"
  bins: 5
  enhet: "Ahus"
  reshID: 104204
...
#dt <- rapbase::loadRegData("NorskHjerteinfarktregister",query=paste("SELECT * FROM hovedskjema_1 WHERE Innleggelsesaar>=2024 AND UnitId=", reshID,";"))
dt <- rapbase::loadRegData("NorskHjerteinfarktregister",query=paste("SELECT * FROM hovedskjema_1 WHERE Innleggelsesaar>=2024;"))
...
Mnd <- as.numeric(substr(dt$Innleggelsestidspunkt,6,7))
...
#kable(tabell)
 ...
 kable(tabell, format = "html",escape=FALSE) %>%
   kable_styling() %>%
...
FirstTimeClosed <- as.POSIXct(dt$FirstTimeClosed,format="%Y-%m-%d%H:%M")
Utreisedato <- as.POSIXct(dt$Utreisedato,format="%Y-%m-%Y%H:%M")
